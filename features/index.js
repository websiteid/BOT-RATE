...
/* ===============================
   BOT CONFIG (EDIT DISINI)
================================= */

const BOT_TOKEN = '7524016177:AAGMKuY_c8CECryDZ_EyQX4184YY3Skg8Xg'  // Ganti dengan token botmu
const ADMIN_ID = 6468926488                                   // Ganti dengan Telegram ID adminmu
const CHANNEL_ID = -1002857800900                             // Ganti dengan ID channelmu (-100 jika supergroup)
...
const userState = new Map()  // Menyimpan state tiap user
const tokenStore = new Map() // Menyimpan PAP token

/* ===============================
   UTILITIES
================================= */

// Generate token 4 digit unik
function genToken() {
  let token
  do {
...
// Ambil username, fallback ke first_name
...
- Akses konten VIP.`,
  ...
  if (state.step === 'rate_emoji') {
    ...
    return bot.sendMessage(chatId, 'Tambah komentar?', {
      reply_markup: {
        keyboard: [['✍️ Kirim Komentar', '🚫 Tidak'], ['⬅️ Kembali']],
        resize_keyboard: true
      }
    })
  }
